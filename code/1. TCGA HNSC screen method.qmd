...
TCGA<-read.csv("df258_sc.csv", header = TRUE) 
...
rand_lymp<-lymp[,-c(1:2)]
rand_lymp<-rand_lymp[,-c(2:3)]


o <- rfsrc(lym_invasion ~ ., data = rand_lymp)
vimp_results <- vimp(o)
#print(vimp_results)

importance_scores <- vimp_results$importance  # Adjust based on actual structure

# Sort the importance scores to identify the top 20 miRNAs
top_miRNAs <- sort(importance_scores, decreasing = TRUE)[1:20]
top_miRNAs
