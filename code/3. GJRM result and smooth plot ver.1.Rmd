...
```{r}
# copula JOE
png("Joe_final_layout_with_labels.png", width = 3000, height = 3500, res = 300)
...
## TAIL DISTRIBUTION WHEN tau value 0.5
```{r}
library(VineCopula)
library(ggplot2)
library(patchwork)
...
model_eq1 <- glm(lymphovascular_invasion_present ~ 
                   hsa_miR_203a_3p + hsa_miR_194_5p + age + 
                   as.factor(clinical_N) +
                   as.factor(margin_status) +
                   as.factor(perineural_invasion_present),
                 family = binomial(),
...
df5_clean$y1_hat <- predict(model_eq1, type = "response")
...
# --- One-stage  ---
Onestage <-  glm(Surv.2_NA ~ lymphovascular_invasion_present + hsa_miR_337_3p + 
    hsa_miR_145_3p + 
    hsa_miR_99a_5p + 
    hsa_miR_146b_3p + 
    age, 
