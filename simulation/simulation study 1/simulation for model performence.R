...
Sumfunc <- function(j, f.list, marg, n, cs, pb) {
  ...
  while (!success) {
    ...
    df_samp <- SurvPHBinSim(100000, cs, pb)
    assign("df_samp", df_samp, envir = .GlobalEnv)
    
    df_clean <- df_samp %>% filter(!is.na(y2))
    #df_clean <- dplyr::filter(df_samp, !is.na(y2))
    assign("df_clean", df_clean, envir = .GlobalEnv)
    
    df_copula <- df_clean %>% sample_n(n)
    assign("df_copula", df_copula, envir = .GlobalEnv)
    
    df<-df_samp %>% sample_n(30000) #adjust sample size for data generating
    assign("df", df, envir = .GlobalEnv)
    
    # Define copula
    Cop <- c("N", "C0", "C90", "C180", "C270", "GAL0", "GAL90", "GAL180", "GAL270", "J0", "J90", "J180", "J270", "G0", "G90", "G180", "G270", "F", "AMH", "FGM", "T", "PL", "HO")[j]####
    
    # 
    out <- gjrm(f.list, data = df_copula, margins = marg, model = "B", copula = Cop, uni.fit=T)
    

    if (check_convergence(out)) {
      msummary <- summary(out)
      
      model_summary <- summary(out)
      
      y1 <- paste(round(model_summary$tableP2[2, 1], 3), "(", paste(round(model_summary$tableP2[2, 1] - (model_summary$tableP2[2, 2] * 1.96), 4), round(model_summary$tableP2[2, 1] + (model_summary$tableP2[2, 2] * 1.96), 4), sep = ","), ")", sep = "")
      
      y1_part <- unlist(strsplit(y1, split = "\\(|\\)", perl = TRUE))
      y1_value <- as.numeric(y1_part[1])
      y1_ci <- as.numeric(strsplit(y1_part[2], split = ",", perl = TRUE)[[1]])
      
      kkk<-k.tau(out)   
      tau_parts <- as.numeric(kkk)
      tau_value <- tau_parts[1]
      low_tau <- tau_parts[2]
      up_tau <- tau_parts[3]
  ...
  # Calculate standard error of att_est
  n1 <- nrow(df)
  se_att_est <- sd(S_t0_y1_1- S_t0_y1_0) / sqrt(n1)
  ...
  margin_error <- z_critical * se_att_est
  
  # Compute confidence interval
  ci_lower <- att_est - margin_error
  ci_upper <- att_est + margin_error
  
  #rm("df", envir = .GlobalEnv)
  #rm("df_copula", envir = .GlobalEnv)
  #rm("df_clean", envir = .GlobalEnv)
  #rm("df_samp", envir = .GlobalEnv)
  
  results<-c(ate_value,low_ate, up_ate, att_est ,ci_lower, ci_upper, AIC(out), BIC(out), tau_value, low_tau,up_tau, y1_value, y1_ci,sum(df$y2 == 0, na.rm = TRUE),sum(df$y2 == 1, na.rm = TRUE),sum(is.na(df$y2))
  )
